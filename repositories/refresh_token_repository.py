...
class RefreshTokenRepository:
	@staticmethod
	async def get_all_by_user_id(session: AsyncSession, user_id: str) -> list[RefreshToken]:
		query = select(RefreshToken).where(RefreshToken.user_id == user_id)
		result = await session.scalars(query)
		return result.all()

	@staticmethod
	async def get_one(session: AsyncSession, user_id: str, device_id: str) -> RefreshToken | None:
	...
	async def create_or_update(
		...
		query = select(RefreshToken).where(RefreshToken.user_id == user_id, RefreshToken.device_id == device_id)
		result = await session.scalars(query)
	...
	async def delete_one(session: AsyncSession, user_id: str, device_id: str) -> None:
		try:
			...
			await session.rollback()
			raise DatabaseError(
				server_message=f'[RefreshToken | delete_one] {error}'
			)
	...
	async def delete_dead(session: AsyncSession, ) -> int:
		try:
			result = await session.execute(
				delete(RefreshToken)
				.where(RefreshToken.exp_time < datetime.now())
			)
			await session.flush()
			...
			await session.rollback()
			raise DatabaseError(
				server_message=f'[RefreshToken | delete_dead] {error}'
			)