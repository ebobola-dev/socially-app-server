...
class TokensService:
	...
	async def generate_pair_and_save_refresh(
		...
		access_exp = datetime.now(timezone.utc) + timedelta(minutes = JWT_CONFIG.ACCESS_DURABILITY_MIN)
		refresh_exp = datetime.now(timezone.utc) + timedelta(days = JWT_CONFIG.REFRESH_DURABILITY_DAYS)
	...
	@staticmethod
	async def get_refresh_by_user_and_device_ids(session: AsyncSession, user_id, device_id):
