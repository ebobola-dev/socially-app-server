...
class BaseModel(AsyncAttrs, DeclarativeBase):
	def to_json(self, safe = False, short=False) -> dict:
		def will_be_added(key) -> bool:
			if short and hasattr(self, '__short_fields__') and key not in self.__short_fields__:
				return False
			if hasattr(self, '__protected_from_json_fields__') and key in self.__protected_from_json_fields__:
				return False
			if hasattr(self, '__safe_fields__') and key in self.__safe_fields__ and not safe:
				return False
			return True

		json_view = {
			col.key: serialize_value(getattr(self, col.key))
			for col in self.__table__.columns if will_be_added(col.key)
		}

		if not short and hasattr(self, '__relationship_fields__') and len(self.__relationship_fields__):
