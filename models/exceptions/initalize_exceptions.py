...
class ConfigNotInitalizedButUsing(InitializeError):
	def __init__(self, config_name: str):
		super().__init__(f'Config [{config_name}] has not been initialized, but is being used')

class UnableToInitalizeDatabase(InitializeError):
	def __init__(self):
		super().__init__(f'Unable to initalize database')

class DatabaseNotInitialized(InitializeError):
	def __init__(self):
		super().__init__(f'Database has not beed initialized, but is being user')