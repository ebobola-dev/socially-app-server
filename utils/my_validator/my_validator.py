...
class ValidateField:
	...
	def email(field_name: str = 'email', required: bool = True, nullable = False):
		return ValidateField(
			...
			required=required,
			nullable=nullable,
			rules=[ReFullmatchPatternRule(re_pattern=RE_PATTERNS.EMAIL, error_description='Invalid Email')],
		)
	...
	@staticmethod
	def date_of_birth(field_name: str = 'date_of_birth', required: bool = False, nullable = False):
		return ValidateField(
			field_name=field_name,
			required=required,
			nullable=nullable,
	...
	@staticmethod
	def avatar_type(field_name: str = 'avatar_type', required: bool = True, nullable: bool = False):
		return ValidateField(
			field_name=field_name,
			required=required,
			nullable=nullable,
	...
	def username(field_name: str = 'username', required: bool = True, nullable = False):
		return ValidateField(
			...
			rules=[
				...
				ReSearchPatternRule(r'^[a-z0-9._]+$', error_description='only lowercase Latin letters, numbers, underscores and dots are allowed'),
				ReSearchPatternRule(r'^[^.]', error_description='cannot start with a dot'),
				ReSearchPatternRule(r'^[^\d]', error_description='cannot start with a number'),
			],
		)
	...
	def password(field_name: str = 'password', required: bool = True, nullable = False):
		return ValidateField(
			...
			rules=[
				IsInstanceRule(str),
				ReFullmatchPatternRule(RE_PATTERNS.PASSWORD, error_description=LENGTH_REQIREMENTS.PASSWORD.TEXT),
			],
		)
...
def validate(
	...
	if not maybe_none and value == None:
		raise MyValidatorError([f"can't be null"], field_name=field_name)
	errors = []
	if value is not None:
		for rule in rules:
			try:
				...
				raise UnableToValidate(field_name=field_name, error=unexcepted_error) from unexcepted_error
	if errors:
		...
		async def wrapper(self, request: Request):
			...
			for field in fields:
				...
				try:
					...
					if not key in field_specific_errors:
